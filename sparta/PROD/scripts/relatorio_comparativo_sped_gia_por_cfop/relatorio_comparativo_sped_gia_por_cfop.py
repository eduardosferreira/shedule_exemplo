...
----------------------------------------------------------------------------------------------
MODULO ...: TESHUVA
SCRIPT ...: relatorio_comparativo_sped_gia_por_cfop.py
CRIACAO ..: 24/03/2021
...
----------------------------------------------------------------------------------------------
  HISTORICO : 
  24/08/2021 - Marcelo Gremonesi - Kyros Consultoria
        Adquacoes para o novo painel 
  03/11/2021 - EDUARDO DA SILVA FERREIRA / KYROS TECNOLOGIA (eduardof@kyros.com.br)
        PTITES-879 - ET - Relatório Análise Sped - Aba Resumo - Inclusão base Isentas e outras
  ...
  17/12/2021 - EDUARDO DA SILVA FERREIRA / KYROS TECNOLOGIA (eduardof@kyros.com.br)
        PTITES-1242 - DEV - Desenvolvimento - Insumo_CFOP_SPED_GIA
        https://jira.telefonica.com.br/browse/PTITES-1242
        necessário revisar o cálculo das colunas VLR_ISENTAS e VLR_OUTRAS a considerar o CST 30 como Isento e não como Outro e ajustar o calculo do valor de Isentos para o CST 20 
...
def retornar_UF(p_IE):
    ...
    l_cursor = sql.geraCnxBD(configuracoes) ## p_connection.cursor()
...
def processa_SPED(l_arquivo):
    l_regrel=[[],[]]
    # PTITES-1242 : Adição dos campos CST
    l_regrel[0]=["CFOP","VLR_OPER","VLR_BASE_ICMS","VLR_ICMS","VLR_ISENTAS","VLR_OUTRAS","TIPO","CST"] 
    ...
    l_VLR_ICMS = float(0)
    l_VLR_ISENTAS = float(0) # PTITES-879
    l_VLR_OUTRAS = float(0) # PTITES-879
    ...
    if (len(l_arquivo) > 4 and os.path.isfile(l_arquivo)) :
        ...
        while (l_linhalida and l_flsair == 0):
            ...
            if (l_tp_reg == "C190"
                ...
                l_VLR_ISENTAS = float(0) # PTITES-879
                l_VLR_OUTRAS = float(0) # PTITES-879
                l_VLR_OPER = float(0) # PTITES-879
                l_VLR_BASE_ICMS = float(0) # PTITES-879
                l_VLR_ICMS = float(0) # PTITES-879
                ...
                if (l_tp_reg == "C190"
                    ...
                    # Inicio PTITES-1242
                    try:
                        l_CST = l_dados_ent[2].upper().strip()
                    except:
                        l_CST = ""
                    # Fim PTITES-1242    
                if l_tp_reg == "C190":
                    # Inicio : PTITES-1242
                    if str(l_dados_ent[2].upper().strip())[-2:] in ("20","30", "40", "41"):
                        l_VLR_ISENTAS = float(l_dados_ent[5].replace(',', '.'))
                        print( 'ISENTAS =', l_VLR_ISENTAS )
                    if str(l_dados_ent[2].upper().strip())[-2:] in ("20"):
                        print( 'l_VLR_ISENTAS - BASE_ICMS =', l_VLR_ISENTAS, '-', l_VLR_BASE_ICMS, '=', l_VLR_ISENTAS - l_VLR_BASE_ICMS )
                        l_VLR_ISENTAS -= l_VLR_BASE_ICMS                        
                    if str(l_dados_ent[2].upper().strip())[-2:] in ("50", "51", "60", "70", "90"):
                        l_VLR_OUTRAS = float(l_dados_ent[5].replace(',', '.'))
                    # Fim : PTITES-1242   
                if l_CFOP.strip():  # PTITES-879
                    l_nregrel += 1 # PTITES-879
                    l_regrel.append([])
                    ...
                    l_regrel[l_nregrel].append(l_VLR_ISENTAS) # PTITES-879
                    l_regrel[l_nregrel].append(l_VLR_OUTRAS) # PTITES-879
                    l_regrel[l_nregrel].append(l_tp_reg)
                    l_regrel[l_nregrel].append(l_CST) # PTITES-1242
...
def main():
    ...
    if not l_ret:
        ...
        and (l_a1 == 1 or l_a2 == 1 or l_a3 == 1 or l_a4 == 1) 
            ...
            v_ob_agrupado_cfop_cst = {} # PTITES-1242
            # Chave do dicionario
            v_cc_chave_cfop_cst = "" # PTITES-1242
            v_cc_cst = "" # PTITES-1242
            ...
            l_REG_VLR_ISENTAS = float(0) # 4 # PTITES-879
            l_REG_VLR_OUTRAS = float(0) # 5 # PTITES-879
            
            l_PROT_SPED = "" # 6
            l_PROT_VLR_OPER  = float(0) # 7
            l_PROT_VLR_BASE_ICMS = float(0) # 8
            l_PROT_VLR_ICMS = float(0) # 9
            l_PROT_VLR_ISENTAS = float(0) # 10 # PTITES-879
            l_PROT_VLR_OUTRAS = float(0) # 11  # PTITES-879          
            
            l_REG_GIA = "" # 12
            l_REG_GIA_VLR_CONTABIL = float(0) # 13
            l_REG_GIA_VLR_BASE_ICMS = float(0) # 14
            l_REG_GIA_VLR_ICMS = float(0) # 15
            l_REG_GIA_VLR_ISENTAS = float(0) # 16
            l_REG_GIA_VLR_OUTRAS = float(0) # 17
            l_REG_GIA_SUBSTITUTO = float(0) # 18
            l_REG_GIA_SUBSTITUIDO = float(0) # 19
            l_REG_GIA_OUTROS_IMPOSTOS = float(0) # 20
            l_PROT_GIA = "" # 21
            l_PROT_GIA_VLR_CONTABIL = float(0) # 22
            l_PROT_GIA_VLR_BASE_ICMS = float(0) # 23
            l_PROT_GIA_VLR_ICMS = float(0) # 24
            l_PROT_GIA_VLR_ISENTAS = float(0) # 25
            l_PROT_GIA_VLR_OUTRAS = float(0) # 26
            l_PROT_GIA_SUBSTITUTO = float(0) # 27
            l_PROT_GIA_SUBSTITUIDO = float(0) # 28
            l_PROT_GIA_OUTROS_IMPOSTOS = float(0) # 29
            ...
            if l_a1 == 1 and v_sped_enxertado is not None:
                ...
                for linha in v_sped_enxertado:
                    ...
                    v_cc_cst = str(linha[7]).strip().upper() # PTITES-1242
                    v_cc_chave_cfop_cst = (str(l_CFOP) , str(v_cc_cst)) # PTITES-1242
                    
                    l_REG_SPED = l_CFOP
                    l_REG_VLR_OPER  = linha[1]
                    l_REG_VLR_BASE_ICMS = linha[2]
                    l_REG_VLR_ICMS = linha[3]
                    l_REG_VLR_ISENTAS = linha[4] # PTITES-879 
                    l_REG_VLR_OUTRAS = linha[5] # PTITES-879
                    
                    # Inicio PTITES-1242
                    # Verifica se existe no dicionario, agrupando por CFOP e CST
                    if v_cc_chave_cfop_cst in v_ob_agrupado_cfop_cst:
                        # Atualiza os valores do dicionario
                        v_ob_agrupado_cfop_cst[v_cc_chave_cfop_cst][0]  = v_cc_chave_cfop_cst
                        v_ob_agrupado_cfop_cst[v_cc_chave_cfop_cst][1] += l_REG_VLR_OPER
                        v_ob_agrupado_cfop_cst[v_cc_chave_cfop_cst][2] += l_REG_VLR_BASE_ICMS
                        v_ob_agrupado_cfop_cst[v_cc_chave_cfop_cst][3] += l_REG_VLR_ICMS
                        v_ob_agrupado_cfop_cst[v_cc_chave_cfop_cst][4] += l_REG_VLR_ISENTAS # PTITES-879
                        v_ob_agrupado_cfop_cst[v_cc_chave_cfop_cst][5] += l_REG_VLR_OUTRAS # PTITES-879
                    else:
                        # Cria uma nova lista e adiciona no dicionario
                        l_lst_aux = []
                        # A lista deve conter estas seguencias 
                        # Refere a lista atual
                        l_lst_aux.append(v_cc_chave_cfop_cst)
                        l_lst_aux.append(l_REG_VLR_OPER)
                        l_lst_aux.append(l_REG_VLR_BASE_ICMS)
                        l_lst_aux.append(l_REG_VLR_ICMS)
                        l_lst_aux.append(l_REG_VLR_ISENTAS) # PTITES-879
                        l_lst_aux.append(l_REG_VLR_OUTRAS) # PTITES-879
                        # Valores default
                        ...
                        l_lst_aux.append(l_PROT_VLR_ISENTAS) # PTITES-879
                        l_lst_aux.append(l_PROT_VLR_OUTRAS) # PTITES-879
                        # Valores default
                        ...
                        l_lst_aux.append(v_cc_chave_cfop_cst)                        
                        v_ob_agrupado_cfop_cst[v_cc_chave_cfop_cst] = l_lst_aux                        
                    # Fim PTITES-1242
                    ...
                    if l_CFOP in l_dicionario:
                        ...
                        l_dicionario[l_CFOP][4] += l_REG_VLR_ISENTAS # PTITES-879
                        l_dicionario[l_CFOP][5] += l_REG_VLR_OUTRAS # PTITES-879
                    else:
                        # Cria uma nova lista e adiciona no dicionario
                        l_lst_aux = []
                        # A lista deve conter estas seguencias 
                    ...
                    l_REG_VLR_ISENTAS = float(0) # PTITES-879
                    l_REG_VLR_OUTRAS = float(0) # PTITES-879
                    # Fim For    
            ...
            if l_a2 == 1 and v_sped_protocolado is not None:
                ...
                for linha in v_sped_protocolado:
                    ...
                    l_PROT_VLR_ISENTAS = linha[4] # PTITES-879 
                    l_PROT_VLR_OUTRAS = linha[5] # PTITES-879
                    # Inicio PTITES-1242
                    # Verifica se existe no dicionario, agrupando por CFOP e CST
                    if v_cc_chave_cfop_cst in v_ob_agrupado_cfop_cst:
                        # Atualiza os valores do dicionario
                        v_ob_agrupado_cfop_cst[v_cc_chave_cfop_cst][6]   = v_cc_chave_cfop_cst # PTITES-879
                        v_ob_agrupado_cfop_cst[v_cc_chave_cfop_cst][7]  += l_PROT_VLR_OPER # PTITES-879
                        v_ob_agrupado_cfop_cst[v_cc_chave_cfop_cst][8]  += l_PROT_VLR_BASE_ICMS # PTITES-879
                        v_ob_agrupado_cfop_cst[v_cc_chave_cfop_cst][9]  += l_PROT_VLR_ICMS # PTITES-879
                        v_ob_agrupado_cfop_cst[v_cc_chave_cfop_cst][10] += l_PROT_VLR_ISENTAS # PTITES-879
                        v_ob_agrupado_cfop_cst[v_cc_chave_cfop_cst][11] += l_PROT_VLR_OUTRAS # PTITES-879                        
                    else:
                        # Cria uma nova lista e adiciona no dicionario
                        l_lst_aux = []
                        # A lista deve conter estas seguencias 
                    ...
                    if l_CFOP in l_dicionario:
                        # Atualiza os valores do dicionario
                        l_dicionario[l_CFOP][6] = l_PROT_SPED # PTITES-879
                        l_dicionario[l_CFOP][7] += l_PROT_VLR_OPER # PTITES-879
                        l_dicionario[l_CFOP][8] += l_PROT_VLR_BASE_ICMS # PTITES-879
                        l_dicionario[l_CFOP][9] += l_PROT_VLR_ICMS # PTITES-879
                        l_dicionario[l_CFOP][10] += l_PROT_VLR_ISENTAS # PTITES-879
                        l_dicionario[l_CFOP][11] += l_PROT_VLR_OUTRAS # PTITES-879                        
                    else:
                        # Cria uma nova lista e adiciona no dicionario
                        l_lst_aux = []
                        # A lista deve conter estas seguencias 
                    ...
                    l_PROT_VLR_ISENTAS = float(0)  # PTITES-879
                    l_PROT_VLR_OUTRAS = float(0)  # PTITES-879                      
                    # Fim For    
            ...
            if l_a3 == 1 and v_gia_enxertado is not None:
                ...
                for linha in v_gia_enxertado:
                    ...
                    if l_CFOP in l_dicionario:
                        # Atualiza os valores do dicionario
                        l_dicionario[l_CFOP][12] = l_REG_GIA # PTITES-879
                        l_dicionario[l_CFOP][13] += l_REG_GIA_VLR_CONTABIL # PTITES-879
                        l_dicionario[l_CFOP][14] += l_REG_GIA_VLR_BASE_ICMS # PTITES-879
                        l_dicionario[l_CFOP][15] += l_REG_GIA_VLR_ICMS # PTITES-879
                        l_dicionario[l_CFOP][16] += l_REG_GIA_VLR_ISENTAS # PTITES-879
                        l_dicionario[l_CFOP][17] += l_REG_GIA_VLR_OUTRAS # PTITES-879
                        l_dicionario[l_CFOP][18] += l_REG_GIA_SUBSTITUTO # PTITES-879
                        l_dicionario[l_CFOP][19] += l_REG_GIA_SUBSTITUIDO # PTITES-879
                        l_dicionario[l_CFOP][20] += l_REG_GIA_OUTROS_IMPOSTOS # PTITES-879
                    else:
                        # Cria uma nova lista e adiciona no dicionario
                        l_lst_aux = []
                        # A lista deve conter estas seguencias 
            ...
            if l_a4 == 1 and v_gia_protocolado is not None:
                ...
                for linha in v_gia_protocolado:
                    ...
                    if l_CFOP in l_dicionario:
                        # Atualiza os valores do dicionario
                        l_dicionario[l_CFOP][21] = l_PROT_GIA # PTITES-879
                        l_dicionario[l_CFOP][22] += l_PROT_GIA_VLR_CONTABIL # PTITES-879
                        l_dicionario[l_CFOP][23] += l_PROT_GIA_VLR_BASE_ICMS # PTITES-879
                        l_dicionario[l_CFOP][24] += l_PROT_GIA_VLR_ICMS # PTITES-879
                        l_dicionario[l_CFOP][25] += l_PROT_GIA_VLR_ISENTAS # PTITES-879
                        l_dicionario[l_CFOP][26] += l_PROT_GIA_VLR_OUTRAS # PTITES-879
                        l_dicionario[l_CFOP][27] += l_PROT_GIA_SUBSTITUTO # PTITES-879
                        l_dicionario[l_CFOP][28] += l_PROT_GIA_SUBSTITUIDO # PTITES-879
                        l_dicionario[l_CFOP][29] += l_PROT_GIA_OUTROS_IMPOSTOS # PTITES-879
                    else:
                        # Cria uma nova lista e adiciona no dicionario
                        l_lst_aux = []
                        # A lista deve conter estas seguencias                        
            ...
            l_ENT_REG_VLR_ISENTAS = float(0) # PTITES-879
            l_ENT_REG_VLR_OUTRAS = float(0) # PTITES-879
            l_ENT_PROT_VLR_OPER  = float(0) 
            l_ENT_PROT_VLR_BASE_ICMS = float(0) 
            l_ENT_PROT_VLR_ICMS = float(0) 
            l_ENT_PROT_VLR_ISENTAS = float(0) # PTITES-879
            l_ENT_PROT_VLR_OUTRAS = float(0) # PTITES-879            
            # Sped - Regerado x Protocolado : SAIDA
            l_SAI_REG_VLR_OPER  = float(0) 
            l_SAI_REG_VLR_BASE_ICMS = float(0) 
            l_SAI_REG_VLR_ICMS = float(0) 
            l_SAI_REG_VLR_ISENTAS = float(0) # PTITES-879
            l_SAI_REG_VLR_OUTRAS = float(0) # PTITES-879            
            l_SAI_PROT_VLR_OPER  = float(0) 
            l_SAI_PROT_VLR_BASE_ICMS = float(0) 
            l_SAI_PROT_VLR_ICMS = float(0) 
            l_SAI_PROT_VLR_ISENTAS = float(0) # PTITES-879
            l_SAI_PROT_VLR_OUTRAS = float(0) # PTITES-879   
            ...
            if len(l_dicionario) > 0:
                try:
                    ...
                    for k in sd: # sorted(l_dicionario, key = l_dicionario.get):
                        ...
                        # Chave
                        l_CFOP = str(k)
                        v_ob_lista_cfop_cst = [v for k,v in v_ob_agrupado_cfop_cst.items() if str(k[0]).strip().upper() == l_CFOP.strip().upper()]
                        ...
                        if l_cont == 1:
                            ...
                            planilha2.cell(l_linha_sped,l_coluna,"PVA SPED FISCAL - REGERADO")
                            planilha2.cell(l_linha_sped,l_coluna).font=Font(bold=True)
                            planilha2.cell(l_linha_sped,l_coluna).alignment = Alignment(horizontal='center')
                            planilha2.merge_cells('B'+ str(l_linha) + ':H' + str(l_linha)) 

                            l_coluna = 10 # # PTITES-1242 PTITES-879  # Coluna G # PVA SPED FISCAL - PROTOCOLADO
                            planilha2.cell(l_linha_sped,l_coluna,"PVA SPED FISCAL - PROTOCOLADO")
                            planilha2.cell(l_linha_sped,l_coluna).font=Font(bold=True)
                            planilha2.cell(l_linha_sped,l_coluna).alignment = Alignment(horizontal='center')
                            planilha2.merge_cells('J'+ str(l_linha) + ':P' + str(l_linha)) # PTITES-879
                            		
                            l_coluna = 18 # # PTITES-1242 PTITES-879  # Coluna L # DIFERENÇA: SPED REGERADO X SPED PROTOCOLADO
                            planilha2.cell(l_linha_sped,l_coluna,"DIFERENÇA: SPED REGERADO X SPED PROTOCOLADO")
                            planilha2.cell(l_linha_sped,l_coluna).font=Font(bold=True)
                            planilha2.cell(l_linha_sped,l_coluna).alignment = Alignment(horizontal='center')
                            planilha2.merge_cells('R'+ str(l_linha) + ':X' + str(l_linha)) # PTITES-879
                            ...
                            planilha3.merge_cells('B'+ str(l_linha) + ':G' + str(l_linha)) # PTITES-879

                            l_coluna = 9 # PTITES-879 # Coluna G # GIA - REGERADO
                            planilha3.cell(l_linha,l_coluna,"GIA - REGERADO")
                            planilha3.cell(l_linha,l_coluna).font=Font(bold=True)
                            planilha3.cell(l_linha,l_coluna).alignment = Alignment(horizontal='center')
                            planilha3.merge_cells('I'+ str(l_linha) + ':N' + str(l_linha)) # PTITES-879
                            		
                            l_coluna = 16 # PTITES-879 # Coluna L # DIFERENÇA: SPED REGERADO X GIA REGERADO
                            planilha3.cell(l_linha,l_coluna,"DIFERENÇA: SPED REGERADO X GIA REGERADO")
                            planilha3.cell(l_linha,l_coluna).font=Font(bold=True)
                            planilha3.cell(l_linha,l_coluna).alignment = Alignment(horizontal='center')
                            planilha3.merge_cells('P'+ str(l_linha) + ':U' + str(l_linha)) # PTITES-879

                            # Inicio PTITES-1242
                            # Campos - Cabeçalho
                            l_linha_sped  += 1 # Linha  2
                            for nCol in (2,10,18): # PTITES-879 
                                l_coluna = nCol # Coluna                                 
                                for nColuna in ("CFOP",
                                                "CST",
                                                "VLR_OPER",	
                                                "VLR_BASE_ICMS",	
                                                "VLR_ICMS",
                                                "VLR_ISENTAS", # PTITES-879            
                                                "VLR_OUTRAS"   # PTITES-879            
                                                ):                                    
                                    # ** Sped - Regerado x Protocolado ***
                                    planilha2.cell(l_linha_sped,l_coluna,nColuna)
                                    planilha2.cell(l_linha_sped,l_coluna).font=Font(bold=True)
                                    planilha2.cell(l_linha_sped,l_coluna).alignment = Alignment(horizontal='center')
                                    l_coluna = l_coluna + 1       
                            # Fim PTITES-1242
                            # Campos - Cabeçalho
                            for nCol in (2,9,16): # PTITES-879 
                                l_coluna = nCol # Coluna 
                                l_linha  = 2 # Linha  2
                                for nColuna in ("CFOP", 
                                                "VLR_OPER",	
                            ...
                            # FIM IF
                        # Inicio PTITES-1242
                        if v_ob_lista_cfop_cst:
                            for v in v_ob_lista_cfop_cst:
                                v_cc_cfop = v[-1][0]
                                v_cc_cst  = v[-1][1]
                                l_linha_sped += 1
                                for l_coluna in (2,10,18):    
                                    planilha2.cell(l_linha_sped,l_coluna,v_cc_cfop) 
                                    planilha2.cell(l_linha_sped,l_coluna).font=Font(bold=True) 
                                    planilha2.cell(l_linha_sped,l_coluna).alignment = Alignment(horizontal='center')
                                    planilha2.cell(l_linha_sped,l_coluna+1,v_cc_cst) 
                                    planilha2.cell(l_linha_sped,l_coluna+1).font=Font(bold=True) 
                                    planilha2.cell(l_linha_sped,l_coluna+1).alignment = Alignment(horizontal='center')
                                l_coluna_sped = 4
                                for n in (1,2,3,4,5,7,8,9,10,11): 
                                    # Inicio For
                                    if n == 7: # PTITES-879
                                        l_coluna_sped = 12
                                    planilha2.cell(l_linha_sped,l_coluna_sped,round(v[n],2))                             
                                    planilha2.cell(l_linha_sped,l_coluna_sped).number_format = "#,##0.00"
                                    if n < 7: 
                                        # Diferença
                                        planilha2.cell(l_linha_sped,l_coluna_sped+16,round(v[n]-v[n+6],2)) # PTITES-879
                                        planilha2.cell(l_linha_sped,l_coluna_sped+16).number_format = "#,##0.00"
                                    l_coluna_sped += 1 
                                    # Fim For
                        else:
                            l_linha_sped += 1
                        ...
                        for nCol in (2,10,9,12,16,18,22): # PTITES-879
                                l_coluna = nCol # Coluna 
                                ...
                                if not v_ob_lista_cfop_cst:# PTITES-1242
                                    if l_coluna in (2,10,18): # # PTITES-1242PTITES-879 
                                        # ** Sped - Regerado x Protocolado ***
                                        planilha2.cell(l_linha_sped,l_coluna,l_CFOP) 
                                        planilha2.cell(l_linha_sped,l_coluna).font=Font(bold=True) 
                                        planilha2.cell(l_linha_sped,l_coluna).alignment = Alignment(horizontal='center')
                                        
                                if l_coluna in (2,9,16): # PTITES-879 
                                    # ** Sped x Gia_Regerado ***
                        ...
                        l_coluna = 3 # Coluna B 
                        l_CFOF_GIA =  l_dicionario[l_CFOP][12].strip() # PTITES-879 
                        for n in (13,14,15,16,17,18,19,20, 22,23,24,25,26,27,28,29): # PTITES-879 
                            # Inicio For
                            if n == 22: # PTITES-879 
                                l_coluna = 13 # PTITES-879# Protocolado 
                                l_CFOF_GIA =  l_dicionario[l_CFOP][21].strip() # PTITES-879
                            ...
                            if n in (13,14,15,16,17): # PTITES-879 
                                planilha3.cell(l_linha,l_coluna+7,round(l_dicionario[l_CFOP][n],2)) # PTITES-879                             
                                planilha3.cell(l_linha,l_coluna+7).number_format = "#,##0.00" # PTITES-879                                

                            if n < 22: # PTITES-879   
                                # Diferença
                                if (len(l_dicionario[l_CFOP][12].strip()) > 0 # PTITES-879 
                                or len(l_dicionario[l_CFOP][21].strip()) > 0 ): # PTITES-879 
                        ...
                        for n in (1,2,3,4,5,7,8,9,10,11): # PTITES-879
                            # Inicio For
                            if n == 7: # PTITES-879
                                l_coluna_sped = 12
                                l_coluna = 10 # PTITES-879 # Protocolado 
                                l_CFOF_SPED = l_dicionario[l_CFOP][6].strip() # PTITES-879
                            
                            if not v_ob_lista_cfop_cst:# PTITES-1242
                                if l_CFOF_SPED is not None and len(l_CFOF_SPED) > 0:
                                    planilha2.cell(l_linha_sped,l_coluna_sped,round(l_dicionario[l_CFOP][n],2))                             
                                planilha2.cell(l_linha_sped,l_coluna_sped).number_format = "#,##0.00"

                            if n < 7: 
                                # Diferença
                                if not v_ob_lista_cfop_cst:
                                    if (len(l_dicionario[l_CFOP][0].strip()) > 0 
                                    or len(l_dicionario[l_CFOP][6].strip()) > 0 ): # PTITES-879
                                        planilha2.cell(l_linha_sped,l_coluna_sped+16,round(l_dicionario[l_CFOP][n]-l_dicionario[l_CFOP][n+6],2)) # PTITES-879
                                    planilha2.cell(l_linha_sped,l_coluna_sped+16).number_format = "#,##0.00"
                                ...
                                planilha3.cell(l_linha,l_coluna+14,round(l_dicionario[l_CFOP][n]-l_dicionario[l_CFOP][n+12],2)) # PTITES-879                             
                                planilha3.cell(l_linha,l_coluna+14).number_format = "#,##0.00"
                        ...
                        if l_CFOP[0] in ('1','2','3'):
                            ...
                            l_ENT_REG_GIA_VLR_CONTABIL += l_dicionario[l_CFOP][13] # PTITES-879
                            l_ENT_REG_GIA_VLR_BASE_ICMS += l_dicionario[l_CFOP][14] # PTITES-879
                            l_ENT_REG_GIA_VLR_ICMS += l_dicionario[l_CFOP][15] # PTITES-879
                            l_ENT_REG_GIA_VLR_ISENTAS += l_dicionario[l_CFOP][16] # PTITES-879
                            l_ENT_REG_GIA_VLR_OUTRAS += l_dicionario[l_CFOP][17] # PTITES-879
                            l_ENT_REG_GIA_SUBSTITUTO += l_dicionario[l_CFOP][18] # PTITES-879
                            l_ENT_REG_GIA_SUBSTITUIDO += l_dicionario[l_CFOP][19] # PTITES-879
                            l_ENT_REG_GIA_OUTROS_IMPOSTOS += l_dicionario[l_CFOP][20] # PTITES-879  
                            
                            l_ENT_PROT_GIA_VLR_CONTABIL += l_dicionario[l_CFOP][22] # PTITES-879
                            l_ENT_PROT_GIA_VLR_BASE_ICMS += l_dicionario[l_CFOP][23] # PTITES-879
                            l_ENT_PROT_GIA_VLR_ICMS += l_dicionario[l_CFOP][24] # PTITES-879
                            l_ENT_PROT_GIA_VLR_ISENTAS += l_dicionario[l_CFOP][25] # PTITES-879
                            l_ENT_PROT_GIA_VLR_OUTRAS += l_dicionario[l_CFOP][26] # PTITES-879
                            l_ENT_PROT_GIA_SUBSTITUTO += l_dicionario[l_CFOP][27] # PTITES-879
                            l_ENT_PROT_GIA_SUBSTITUIDO += l_dicionario[l_CFOP][28] # PTITES-879
                            l_ENT_PROT_GIA_OUTROS_IMPOSTOS += l_dicionario[l_CFOP][29] # PTITES-879
                            # Sped - Regerado x Protocolado
                            ...
                            l_ENT_REG_VLR_ISENTAS += l_dicionario[l_CFOP][4]  # PTITES-879
                            l_ENT_REG_VLR_OUTRAS += l_dicionario[l_CFOP][5]  # PTITES-879            
                            l_ENT_PROT_VLR_OPER += l_dicionario[l_CFOP][7]  # PTITES-879
                            l_ENT_PROT_VLR_BASE_ICMS += l_dicionario[l_CFOP][8]  # PTITES-879
                            l_ENT_PROT_VLR_ICMS += l_dicionario[l_CFOP][9] # PTITES-879 
                            l_ENT_PROT_VLR_ISENTAS += l_dicionario[l_CFOP][10] # PTITES-879 
                            l_ENT_PROT_VLR_OUTRAS += l_dicionario[l_CFOP][11] # PTITES-879          
            
                        # Somatorio de SAÍDA
                        if l_CFOP[0] in ('5','6','7'):
                             #  GIA - Regerado x Protocolado     
                            l_SAI_REG_GIA_VLR_CONTABIL += l_dicionario[l_CFOP][13] # PTITES-879
                            l_SAI_REG_GIA_VLR_BASE_ICMS += l_dicionario[l_CFOP][14] # PTITES-879
                            l_SAI_REG_GIA_VLR_ICMS += l_dicionario[l_CFOP][15] # PTITES-879
                            l_SAI_REG_GIA_VLR_ISENTAS += l_dicionario[l_CFOP][16] # PTITES-879
                            l_SAI_REG_GIA_VLR_OUTRAS += l_dicionario[l_CFOP][17] # PTITES-879
                            l_SAI_REG_GIA_SUBSTITUTO += l_dicionario[l_CFOP][18] # PTITES-879
                            l_SAI_REG_GIA_SUBSTITUIDO += l_dicionario[l_CFOP][19] # PTITES-879
                            l_SAI_REG_GIA_OUTROS_IMPOSTOS += l_dicionario[l_CFOP][20] # PTITES-879   
                            l_SAI_PROT_GIA_VLR_CONTABIL += l_dicionario[l_CFOP][22] # PTITES-879
                            l_SAI_PROT_GIA_VLR_BASE_ICMS += l_dicionario[l_CFOP][23] # PTITES-879
                            l_SAI_PROT_GIA_VLR_ICMS += l_dicionario[l_CFOP][24] # PTITES-879
                            l_SAI_PROT_GIA_VLR_ISENTAS += l_dicionario[l_CFOP][25] # PTITES-879
                            l_SAI_PROT_GIA_VLR_OUTRAS += l_dicionario[l_CFOP][26] # PTITES-879
                            l_SAI_PROT_GIA_SUBSTITUTO += l_dicionario[l_CFOP][27] # PTITES-879
                            l_SAI_PROT_GIA_SUBSTITUIDO += l_dicionario[l_CFOP][28] # PTITES-879
                            l_SAI_PROT_GIA_OUTROS_IMPOSTOS += l_dicionario[l_CFOP][29] # PTITES-879
                            # Sped - Regerado x Protocolado
                            ...
                            l_SAI_REG_VLR_ISENTAS += l_dicionario[l_CFOP][4] # PTITES-879
                            l_SAI_REG_VLR_OUTRAS += l_dicionario[l_CFOP][5] # PTITES-879           
            
                            l_SAI_PROT_VLR_OPER += l_dicionario[l_CFOP][7] # PTITES-879
                            l_SAI_PROT_VLR_BASE_ICMS += l_dicionario[l_CFOP][8] # PTITES-879
                            l_SAI_PROT_VLR_ICMS += l_dicionario[l_CFOP][9] # PTITES-879
                            l_SAI_PROT_VLR_ISENTAS += l_dicionario[l_CFOP][10]  # PTITES-879
                            l_SAI_PROT_VLR_OUTRAS += l_dicionario[l_CFOP][11]  # PTITES-879   
                    ...
                    if l_cont > 0:
                        # TOTAL DE ENTRADA
                        l_linha_sped += 2 # PTITES-1242
                        l_linha += 2
                        l_coluna = 2 # Coluna B 
                        planilha1.cell(l_linha,l_coluna,"TOTAL ENTRADA") 
                        planilha1.cell(l_linha,l_coluna).font=Font(bold=True)
                        planilha1.cell(l_linha,l_coluna).alignment = Alignment(horizontal='center')
                        planilha2.cell(l_linha_sped,l_coluna,"TOTAL ENTRADA") 
                        planilha2.cell(l_linha_sped,l_coluna).font=Font(bold=True)
                        planilha2.cell(l_linha_sped,l_coluna).alignment = Alignment(horizontal='center')                        
                        ...
                        # Sped - Regerado x Protocolado
                        l_coluna = 4 # PTITES-1242
                        planilha2.cell(l_linha_sped,l_coluna,round(l_ENT_REG_VLR_OPER,2)) 
                        planilha2.cell(l_linha_sped,l_coluna).number_format = "#,##0.00"    
                        planilha2.cell(l_linha_sped,l_coluna).font=Font(bold=True)                    

                        l_coluna += 1
                        planilha2.cell(l_linha_sped,l_coluna,round(l_ENT_REG_VLR_BASE_ICMS,2)) 
                        planilha2.cell(l_linha_sped,l_coluna).number_format = "#,##0.00"
                        planilha2.cell(l_linha_sped,l_coluna).font=Font(bold=True)  

                        l_coluna += 1
                        planilha2.cell(l_linha_sped,l_coluna,round(l_ENT_REG_VLR_ICMS,2)) 
                        planilha2.cell(l_linha_sped,l_coluna).number_format = "#,##0.00"
                        planilha2.cell(l_linha_sped,l_coluna).font=Font(bold=True)  
                        
                        l_coluna += 1 # # PTITES-1242 PTITES-879
                        planilha2.cell(l_linha_sped,l_coluna,round(l_ENT_REG_VLR_ISENTAS,2)) # PTITES-879
                        planilha2.cell(l_linha_sped,l_coluna).number_format = "#,##0.00" # PTITES-879
                        planilha2.cell(l_linha_sped,l_coluna).font=Font(bold=True)   # PTITES-879
                        
                        l_coluna += 1 # # PTITES-1242 PTITES-879
                        planilha2.cell(l_linha_sped,l_coluna,round(l_ENT_REG_VLR_OUTRAS,2))  # PTITES-879
                        planilha2.cell(l_linha_sped,l_coluna).number_format = "#,##0.00" # PTITES-879
                        planilha2.cell(l_linha_sped,l_coluna).font=Font(bold=True)  # PTITES-879

                        l_coluna = 12 # # PTITES-1242 PTITES-879 # PROTOCOLADO
                        planilha2.cell(l_linha_sped,l_coluna,round(l_ENT_PROT_VLR_OPER,2)) 
                        planilha2.cell(l_linha_sped,l_coluna).number_format = "#,##0.00"
                        planilha2.cell(l_linha_sped,l_coluna).font=Font(bold=True)  

                        l_coluna += 1
                        planilha2.cell(l_linha_sped,l_coluna,round(l_ENT_PROT_VLR_BASE_ICMS,2)) 
                        planilha2.cell(l_linha_sped,l_coluna).number_format = "#,##0.00"
                        planilha2.cell(l_linha_sped,l_coluna).font=Font(bold=True)  

                        l_coluna += 1
                        planilha2.cell(l_linha_sped,l_coluna,round(l_ENT_PROT_VLR_ICMS,2)) 
                        planilha2.cell(l_linha_sped,l_coluna).number_format = "#,##0.00"
                        planilha2.cell(l_linha_sped,l_coluna).font=Font(bold=True)  

                        l_coluna += 1 # # PTITES-1242 PTITES-879
                        planilha2.cell(l_linha_sped,l_coluna,round(l_ENT_PROT_VLR_ISENTAS,2)) # PTITES-879
                        planilha2.cell(l_linha_sped,l_coluna).number_format = "#,##0.00" # PTITES-879
                        planilha2.cell(l_linha_sped,l_coluna).font=Font(bold=True)  # PTITES-879
                        
                        l_coluna += 1 # # PTITES-1242 PTITES-879
                        planilha2.cell(l_linha_sped,l_coluna,round(l_ENT_PROT_VLR_OUTRAS,2)) # PTITES-879
                        planilha2.cell(l_linha_sped,l_coluna).number_format = "#,##0.00" # PTITES-879
                        planilha2.cell(l_linha_sped,l_coluna).font=Font(bold=True) # PTITES-879 
                        
                        l_coluna = 20 # # PTITES-1242 PTITES-879 # DIFERENCA
                        planilha2.cell(l_linha_sped,l_coluna,round(l_ENT_REG_VLR_OPER-l_ENT_PROT_VLR_OPER,2)) 
                        planilha2.cell(l_linha_sped,l_coluna).number_format = "#,##0.00"    
                        planilha2.cell(l_linha_sped,l_coluna).font=Font(bold=True)                    

                        l_coluna += 1 # PTITES-1242
                        planilha2.cell(l_linha_sped,l_coluna,round(l_ENT_REG_VLR_BASE_ICMS-l_ENT_PROT_VLR_BASE_ICMS,2)) 
                        planilha2.cell(l_linha_sped,l_coluna).number_format = "#,##0.00"
                        planilha2.cell(l_linha_sped,l_coluna).font=Font(bold=True)  

                        l_coluna += 1 # PTITES-1242
                        planilha2.cell(l_linha_sped,l_coluna,round(l_ENT_REG_VLR_ICMS-l_ENT_PROT_VLR_ICMS,2)) 
                        planilha2.cell(l_linha_sped,l_coluna).number_format = "#,##0.00"
                        planilha2.cell(l_linha_sped,l_coluna).font=Font(bold=True)  

                        l_coluna += 1 # PTITES-1242 # PTITES-879
                        planilha2.cell(l_linha_sped,l_coluna,round(l_ENT_REG_VLR_OUTRAS-l_ENT_PROT_VLR_OUTRAS,2))  # PTITES-879
                        planilha2.cell(l_linha_sped,l_coluna).number_format = "#,##0.00" # PTITES-879
                        planilha2.cell(l_linha_sped,l_coluna).font=Font(bold=True)  # PTITES-879
                        
                        l_coluna += 1 # PTITES-1242 # PTITES-879
                        planilha2.cell(l_linha_sped,l_coluna,round(l_ENT_REG_VLR_ISENTAS-l_ENT_PROT_VLR_ISENTAS,2)) # PTITES-879
                        planilha2.cell(l_linha_sped,l_coluna).number_format = "#,##0.00" # PTITES-879
                        planilha2.cell(l_linha_sped,l_coluna).font=Font(bold=True)  # PTITES-879
                        ...
                        l_coluna += 1 # PTITES-879
                        planilha3.cell(l_linha,l_coluna,round(l_ENT_REG_VLR_ISENTAS,2)) # PTITES-879
                        planilha3.cell(l_linha,l_coluna).number_format = "#,##0.00" # PTITES-879
                        planilha3.cell(l_linha,l_coluna).font=Font(bold=True) # PTITES-879  
                        
                        l_coluna += 1 # PTITES-879
                        planilha3.cell(l_linha,l_coluna,round(l_ENT_REG_VLR_OUTRAS,2)) # PTITES-879
                        planilha3.cell(l_linha,l_coluna).number_format = "#,##0.00" # PTITES-879
                        planilha3.cell(l_linha,l_coluna).font=Font(bold=True)  # PTITES-879
                        
                        # GIA
                        l_coluna = 10 # PTITES-879
                        ...
                        l_coluna += 1 # PTITES-879
                        planilha3.cell(l_linha,l_coluna,round(l_ENT_REG_GIA_VLR_ISENTAS,2)) # PTITES-879
                        planilha3.cell(l_linha,l_coluna).number_format = "#,##0.00" # PTITES-879
                        planilha3.cell(l_linha,l_coluna).font=Font(bold=True) # PTITES-879
                        
                        l_coluna += 1 # PTITES-879
                        planilha3.cell(l_linha,l_coluna,round(l_ENT_REG_GIA_VLR_OUTRAS,2)) # PTITES-879
                        planilha3.cell(l_linha,l_coluna).number_format = "#,##0.00" # PTITES-879
                        planilha3.cell(l_linha,l_coluna).font=Font(bold=True) # PTITES-879
                        
                        # DIFERENCA
                        l_coluna = 17 # PTITES-879
                        ...
                        l_coluna += 1 # PTITES-879
                        planilha3.cell(l_linha,l_coluna,round(l_ENT_REG_VLR_ISENTAS-l_ENT_REG_GIA_VLR_ISENTAS,2)) # PTITES-879
                        planilha3.cell(l_linha,l_coluna).number_format = "#,##0.00" # PTITES-879
                        planilha3.cell(l_linha,l_coluna).font=Font(bold=True)  # PTITES-879

                        l_coluna += 1 # PTITES-879
                        planilha3.cell(l_linha,l_coluna,round(l_ENT_REG_VLR_OUTRAS-l_ENT_REG_GIA_VLR_OUTRAS,2)) # PTITES-879
                        planilha3.cell(l_linha,l_coluna).number_format = "#,##0.00" # PTITES-879
                        planilha3.cell(l_linha,l_coluna).font=Font(bold=True)  # PTITES-879
                        ...
                        planilha1.cell(l_linha,l_coluna).font=Font(bold=True) 
                        planilha1.cell(l_linha,l_coluna).alignment = Alignment(horizontal='center')
                        planilha2.cell(l_linha_sped,l_coluna,"TOTAL SAIDA") # Coluna L # Gia (Protocolado)
                        planilha2.cell(l_linha_sped,l_coluna).font=Font(bold=True) 
                        planilha2.cell(l_linha_sped,l_coluna).alignment = Alignment(horizontal='center')
                        ...
                        # Sped - Regerado x Protocolado
                        l_coluna = 4 # PTITES-1242
                        planilha2.cell(l_linha_sped,l_coluna,round(l_SAI_REG_VLR_OPER,2)) 
                        planilha2.cell(l_linha_sped,l_coluna).number_format = "#,##0.00"    
                        planilha2.cell(l_linha_sped,l_coluna).font=Font(bold=True)                    

                        l_coluna += 1
                        planilha2.cell(l_linha_sped,l_coluna,round(l_SAI_REG_VLR_BASE_ICMS,2)) 
                        planilha2.cell(l_linha_sped,l_coluna).number_format = "#,##0.00"
                        planilha2.cell(l_linha_sped,l_coluna).font=Font(bold=True)  

                        l_coluna += 1
                        planilha2.cell(l_linha_sped,l_coluna,round(l_SAI_REG_VLR_ICMS,2)) 
                        planilha2.cell(l_linha_sped,l_coluna).number_format = "#,##0.00"
                        planilha2.cell(l_linha_sped,l_coluna).font=Font(bold=True)  

                        l_coluna += 1 # PTITES-1242 # PTITES-879
                        planilha2.cell(l_linha_sped,l_coluna,round(l_SAI_REG_VLR_ISENTAS,2)) # PTITES-879 
                        planilha2.cell(l_linha_sped,l_coluna).number_format = "#,##0.00" # PTITES-879
                        planilha2.cell(l_linha_sped,l_coluna).font=Font(bold=True) # PTITES-879  
                        
                        l_coluna += 1 # PTITES-1242 # PTITES-879
                        planilha2.cell(l_linha_sped,l_coluna,round(l_SAI_REG_VLR_OUTRAS,2)) # PTITES-879 
                        planilha2.cell(l_linha_sped,l_coluna).number_format = "#,##0.00" # PTITES-879
                        planilha2.cell(l_linha_sped,l_coluna).font=Font(bold=True) # PTITES-879  
                        
                        l_coluna = 12 # PTITES-1242 # PTITES-879  # PROTOCOLADO
                        planilha2.cell(l_linha_sped,l_coluna,round(l_SAI_PROT_VLR_OPER,2)) 
                        planilha2.cell(l_linha_sped,l_coluna).number_format = "#,##0.00"
                        planilha2.cell(l_linha_sped,l_coluna).font=Font(bold=True)  

                        l_coluna += 1 # PTITES-1242
                        planilha2.cell(l_linha_sped,l_coluna,round(l_SAI_PROT_VLR_BASE_ICMS,2)) 
                        planilha2.cell(l_linha_sped,l_coluna).number_format = "#,##0.00"
                        planilha2.cell(l_linha_sped,l_coluna).font=Font(bold=True)  

                        l_coluna += 1 # PTITES-1242
                        planilha2.cell(l_linha_sped,l_coluna,round(l_SAI_PROT_VLR_ICMS,2)) 
                        planilha2.cell(l_linha_sped,l_coluna).number_format = "#,##0.00"
                        planilha2.cell(l_linha_sped,l_coluna).font=Font(bold=True)  

                        l_coluna += 1 # PTITES-1242 # PTITES-879
                        planilha2.cell(l_linha_sped,l_coluna,round(l_SAI_PROT_VLR_ISENTAS,2)) # PTITES-879
                        planilha2.cell(l_linha_sped,l_coluna).number_format = "#,##0.00" # PTITES-879
                        planilha2.cell(l_linha_sped,l_coluna).font=Font(bold=True) # PTITES-879  

                        l_coluna += 1 # PTITES-1242 # PTITES-879
                        planilha2.cell(l_linha_sped,l_coluna,round(l_SAI_PROT_VLR_OUTRAS,2)) # PTITES-879
                        planilha2.cell(l_linha_sped,l_coluna).number_format = "#,##0.00" # PTITES-879
                        planilha2.cell(l_linha_sped,l_coluna).font=Font(bold=True) # PTITES-879  
               
                        l_coluna = 20 # PTITES-1242 # PTITES-879 # DIFERENCA
                        planilha2.cell(l_linha_sped,l_coluna,round(l_SAI_REG_VLR_OPER-l_SAI_PROT_VLR_OPER,2)) 
                        planilha2.cell(l_linha_sped,l_coluna).number_format = "#,##0.00"    
                        planilha2.cell(l_linha_sped,l_coluna).font=Font(bold=True)                    

                        l_coluna += 1 # PTITES-1242
                        planilha2.cell(l_linha_sped,l_coluna,round(l_SAI_REG_VLR_BASE_ICMS-l_SAI_PROT_VLR_BASE_ICMS,2)) 
                        planilha2.cell(l_linha_sped,l_coluna).number_format = "#,##0.00"
                        planilha2.cell(l_linha_sped,l_coluna).font=Font(bold=True)  

                        l_coluna += 1 # PTITES-1242
                        planilha2.cell(l_linha_sped,l_coluna,round(l_SAI_REG_VLR_ICMS-l_SAI_PROT_VLR_ICMS,2)) 
                        planilha2.cell(l_linha_sped,l_coluna).number_format = "#,##0.00"
                        planilha2.cell(l_linha_sped,l_coluna).font=Font(bold=True)  

                        l_coluna += 1 # PTITES-1242 # PTITES-879
                        planilha2.cell(l_linha_sped,l_coluna,round(l_SAI_REG_VLR_ISENTAS-l_SAI_PROT_VLR_ISENTAS,2)) # PTITES-879
                        planilha2.cell(l_linha_sped,l_coluna).number_format = "#,##0.00" # PTITES-879
                        planilha2.cell(l_linha_sped,l_coluna).font=Font(bold=True) # PTITES-879

                        l_coluna += 1 # PTITES-1242 # PTITES-879
                        planilha2.cell(l_linha_sped,l_coluna,round(l_SAI_REG_VLR_OUTRAS-l_SAI_PROT_VLR_OUTRAS,2)) # PTITES-879
                        planilha2.cell(l_linha_sped,l_coluna).number_format = "#,##0.00" # PTITES-879
                        planilha2.cell(l_linha_sped,l_coluna).font=Font(bold=True)  # PTITES-879
                        ...
                        l_coluna += 1 # PTITES-879
                        planilha3.cell(l_linha,l_coluna,round(l_SAI_REG_VLR_ISENTAS,2)) # PTITES-879
                        planilha3.cell(l_linha,l_coluna).number_format = "#,##0.00" # PTITES-879
                        planilha3.cell(l_linha,l_coluna).font=Font(bold=True) # PTITES-879     

                        l_coluna += 1 # PTITES-879
                        planilha3.cell(l_linha,l_coluna,round(l_SAI_REG_VLR_OUTRAS,2)) # PTITES-879
                        planilha3.cell(l_linha,l_coluna).number_format = "#,##0.00" # PTITES-879
                        planilha3.cell(l_linha,l_coluna).font=Font(bold=True) # PTITES-879    
                        ...
                        l_coluna += 1 # PTITES-879
                        planilha3.cell(l_linha,l_coluna,round(l_SAI_REG_GIA_VLR_ISENTAS,2)) # PTITES-879
                        planilha3.cell(l_linha,l_coluna).number_format = "#,##0.00" # PTITES-879
                        planilha3.cell(l_linha,l_coluna).font=Font(bold=True) # PTITES-879     
                        
                        l_coluna += 1 # PTITES-879
                        planilha3.cell(l_linha,l_coluna,round(l_SAI_REG_GIA_VLR_OUTRAS,2)) # PTITES-879
                        planilha3.cell(l_linha,l_coluna).number_format = "#,##0.00" # PTITES-879
                        planilha3.cell(l_linha,l_coluna).font=Font(bold=True)   # PTITES-879  
                        
                        l_coluna = 17 # PTITES-879 # DIFERENCA                                     
                        ...
                        l_coluna += 1 # PTITES-879
                        planilha3.cell(l_linha,l_coluna,round(l_SAI_REG_VLR_ISENTAS-l_SAI_REG_GIA_VLR_ISENTAS,2)) # PTITES-879
                        planilha3.cell(l_linha,l_coluna).number_format = "#,##0.00" # PTITES-879
                        planilha3.cell(l_linha,l_coluna).font=Font(bold=True)  # PTITES-879

                        l_coluna += 1 # PTITES-879
                        planilha3.cell(l_linha,l_coluna,round(l_SAI_REG_VLR_OUTRAS-l_SAI_REG_GIA_VLR_OUTRAS,2)) # PTITES-879
                        planilha3.cell(l_linha,l_coluna).number_format = "#,##0.00" # PTITES-879
                        planilha3.cell(l_linha,l_coluna).font=Font(bold=True) # PTITES-879                                                 
    ...
    try:
        ...
        if not ret :
            try:
                ...
                gv_diretorio_sped_fiscal_enxertado = configuracoes.sped_fiscal_enxertado.replace("<<MM>>",gv_mes).replace("<<AAAA>>",gv_ano).replace("<<UF>>",gv_uf).replace("<<IE>>",gv_ie).replace("<<MESANO>>",gv_mes_ano).strip()
                gv_diretorio_sped_fiscal_protocolado = configuracoes.sped_fiscal_protocolado.replace("<<MM>>",gv_mes).replace("<<AAAA>>",gv_ano).replace("<<UF>>",gv_uf).replace("<<IE>>",gv_ie).replace("<<MESANO>>",gv_mes_ano).strip()
                gv_diretorio_gia_enxertado = configuracoes.gia_enxertado.replace("<<MM>>",gv_mes).replace("<<AAAA>>",gv_ano).replace("<<UF>>",gv_uf).replace("<<IE>>",gv_ie).replace("<<MESANO>>",gv_mes_ano).strip()
                gv_diretorio_gia_protocolado = configuracoes.gia_protocolado.replace("<<MM>>",gv_mes).replace("<<AAAA>>",gv_ano).replace("<<UF>>",gv_uf).replace("<<IE>>",gv_ie).replace("<<MESANO>>",gv_mes_ano).strip()
                gv_diretorio_insumo_sped = configuracoes.insumo_sped.replace("<<MM>>",gv_mes).replace("<<AAAA>>",gv_ano).replace("<<UF>>",gv_uf).replace("<<IE>>",gv_ie).replace("<<MESANO>>",gv_mes_ano).strip()
                
                gv_arq_sped_fiscal_enxertado = configuracoes.arq_sped_fiscal_enxertado.replace("<<MM>>",gv_mes).replace("<<AAAA>>",gv_ano).replace("<<UF>>",gv_uf).replace("<<IE>>",gv_ie).replace("<<MESANO>>",gv_mes_ano).replace("<<NNN>>","*").strip()
                gv_arq_sped_fiscal_protocolado = configuracoes.arq_sped_fiscal_protocolado.replace("<<MM>>",gv_mes).replace("<<AAAA>>",gv_ano).replace("<<UF>>",gv_uf).replace("<<IE>>",gv_ie).replace("<<MESANO>>",gv_mes_ano).replace("<<NNN>>","*").strip()
                gv_arq_gia_enxertado = configuracoes.arq_gia_enxertado.replace("<<MM>>",gv_mes).replace("<<AAAA>>",gv_ano).replace("<<UF>>",gv_uf).replace("<<IE>>",gv_ie).replace("<<MESANO>>",gv_mes_ano).replace("<<NNN>>","*").strip()
                gv_arq_gia_protocolado = configuracoes.arq_gia_protocolado.replace("<<MM>>",gv_mes).replace("<<AAAA>>",gv_ano).replace("<<UF>>",gv_uf).replace("<<IE>>",gv_ie).replace("<<MESANO>>",gv_mes_ano).replace("<<NNN>>","*").strip()
                gv_arq_insumo_sped = configuracoes.arq_insumo_sped.replace("<<MM>>",gv_mes).replace("<<AAAA>>",gv_ano).replace("<<UF>>",gv_uf).replace("<<IE>>",gv_ie).replace("<<MESANO>>",gv_mes_ano).replace("<<NNN>>","*").strip()
