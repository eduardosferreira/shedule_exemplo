...
def busca_filiais(uf,fils,datai):
    ...
    if fils == "('TODOS')":
        query = """SELECT DISTINCT serie, fili_cod, id_serie_levantamento
                     FROM  gfcarga.TSH_SERIE_LEVANTAMENTO%s s
                    WHERE s.uf_filial = '%s'
                      AND s.EMPS_COD = 'TBRA'
                      AND s.mes_ano  = TO_DATE('%s','dd/mm/yyyy')
                """%(c2,uf, datai)
    else:
        query = """SELECT DISTINCT serie, fili_cod, id_serie_levantamento
                     FROM  gfcarga.TSH_SERIE_LEVANTAMENTO%s s
...
def processar():
    ufi     = comum.getParametro('uf')
    mesanoi = comum.getParametro('mes_ano')
    mesi    = mesanoi[:2]
    anoi    = mesanoi[2:]
    seriesi = comum.getParametro('serie')
    ret     = 0
    configuracoes.var05  
   
    if util.validauf(ufi) and len(mesanoi) == 6 and int(mesi) > 0 and int(mesi) < 13 and int(anoi) <= datetime.datetime.now().year and int(anoi) > (datetime.datetime.now().year)-50:
        anoR    = mesanoi[4:]
        datai   = "01/" + mesi + "/" + anoi
        dataf   =  str(ultimodia(int(anoi),int(mesi)))+"/"+str(mesi)+"/"+str(anoi)

        lista_series = [x.strip() for x in seriesi.split(',')]                    

        # for x in lista_series:
        #     x = x.upper()
        #     if x.replace(' ','') not in ('1','C','UT','UK','TODOS'):
        #         ret = 99
        #         log('#### - Relatório não gerado, ERRO: ' + str(ret))
        #         log('SERIE INVÁLIDA, ' + x)
        #         log.ret = 99
        #         return ret  

        string_lista_series = "("            
        for x in range(len(lista_series)):
            string_lista_series = string_lista_series + "'" + lista_series[x].strip() + "', "
        if (len(string_lista_series) > 3):
            string_lista_series = string_lista_series[:len(string_lista_series)-2]
            string_lista_series = string_lista_series + ")"
        log("Lista de séries: " ,string_lista_series)
        lista_series = string_lista_series
        lista_series = lista_series.upper()        
    else:
        comum.imprimeHelp()
        return ret
   
    log("# Buscando séries para a UF informada...", ufi)
    filiais = busca_filiais(ufi,lista_series,datai)
    log("# Fim da lista de séries.")
    log("# SERIES: ", filiais)
    ...
    for val in filiais:
        log('SÉRIE       = ' , str(val[0]))
        log('FILIAL      = ' , str(val[1]))
        log('IE          = ' , str(val[2]))
        log('DATA INICIO = ' , datai)
        log('DATA FIM    = ' , dataf)
        
        log("# Iniciando busca de dados para o relatório com a série " + str(val[0]))

        #### Monta caminho e nome do relatorio MUDAR PORTALOPTRB
        dir_base_r = SD + 'portaloptrib' + SD + 'LEVCV115' + SD + ufi + SD + anoR + SD + mesi + SD + 'TBRA' + SD + str(val[1]) + SD + 'SERIE' + SD + str(val[2]) + SD + 'INSUMOS'
        # dir_base_r = SD + 'arquivos' + SD + 'LEVCV115' + SD + ufi + SD + anoR + SD + mesi + SD + 'TBRA' + SD + str(val[1]) + SD + 'SERIE' + SD + str(val[2]) + SD + 'INSUMOS'
        dir_relatorio = os.path.join(dir_base_r)    
        ...
        impressao = insere_impressao(datai,dataf,"'"+val[0]+"'",ufi)
        if impressao > 0:
            log("####")
        ...
        protocolado = insere_protocolado(datai,dataf,"'"+val[0]+"'",ufi)
        if protocolado > 0:
            log("####")
        ...
        dados_relatorio = busca_relatorio(datai,dataf,"'"+val[0]+"'",ufi)
        ...
        for row in dados_relatorio[1:]: 
            ...
            if not relatorio or count == 500000:
                ...
                nome_arquivo = "Comparativo_Conv115_Original_x_Impressao_x_Regerado_"+ufi+"_"+val[0].replace(' ', '')+"_"+mesanoi+"_"+str(volume).rjust(3,'0')+".csv"
...
if __name__ == "__main__":
    cod_saida = 0
    log('-'*100)
    comum.addParametro('uf', None, 'Sigla UF em maiusculo', True, 'SP', None)
    comum.addParametro('mes_ano', None, 'Periodo no formato MMYYYY', True, '012016', None)
    comum.addParametro('serie', None, 'Séries 1, C, U  T, UK', False, 'C', '1, C, U  T, UK')
    comum.addParametro('flagLimite', None, 'Limita o retorno de linhas em até 10000', False, 'N', 'N')
