...
def todaspastas(CC_IE,CC_FILIAL,DT_MESANO_INICIO,DT_MESANO_FIM,CC_SERIE,UFI="SP") :
    ret = 0
    ...
    for dirobrig in lst_path_temp:
        print("Verificando pasta: ", dirobrig)
    ...
    print("SOMENTES ESTAS PASTAS ATENDEM AOS PARAMETROS INFORMADOS:")
    lst_obrig_mov = []
    ...
    for dirobrig in lst_obrig_mov:
        ...
        try:
            ...
            print("movendo ", dirobrig, " para ", dirobrigold)
            input("Move ?")
            shutil.move(dirobrig,dirobrigold)
                
            os.mkdir(dirobrig)     
            origem = dirobrig.replace(dirobrig,"ULTIMA_ENTREGA")

            maskfiles=os.path.join(origem,"*M.[0-9][0-9][0-9]")
            for file in glob.glob(maskfiles):
                destino = file.replace(configuracoes.OBRIGACAO,"OBRIGACAO")
                log("Copiando ", file, " para ", destino, " ...")
                shutil.copy2(file,destino)
            
            maskfiles=os.path.join(origem,"*I.[0-9][0-9][0-9]")
            for file in glob.glob(maskfiles):
                destino = file.replace(configuracoes.OBRIGACAO,"OBRIGACAO")
                log("Copiando ", file, " para ", destino, " ...")
                shutil.copy2(file,destino)

            maskfiles=os.path.join(origem,"*D.[0-9][0-9][0-9]")
            for file in glob.glob(maskfiles):
                destino = file.replace(configuracoes.OBRIGACAO,"OBRIGACAO")
                log("Copiando ", file, " para ", destino, " ...")
                shutil.copy2(file,destino)
    ...
    for tipoarq in ("*C","*1","*UT","*UK","*Z01","*Z02","*Z03","*Z04","*V??"):
        maskoutros = os.path.join(origem, tipoarq+".[0-9][0-9][0-9]")
        for arqerro in glob.glob(maskoutros):
            log("ERRO - O arquivo ", arqerro ," não foi copiado de ULTIMA_ENTREGA PARA OBRIGAÇÃO")   
            ret = 99
...
for file in glob.glob(r'C:/*.txt'):
    ...
    input("Listou todos os diretorios OBRIGACAO")
    ...
    for dat in lista_datas:
        print("Montando path para dat = ", dat)
        mask = os.path.join(pasta,UFI,dat[0:2].zfill(2),dat[3:5].zfill(2),"TBRA",CC_FILIAL.replace("'",""),"SERIE")                 
        lst_path_regerado.append(str(mask))
...
def selectPastas(CC_IE,CC_FILIAL,DT_MESANO_INICIO,DT_MESANO_FIM,CC_SERIE,UFI="SP") :
    ...
    print("ANTES DO IF, CC_SERIE = ", CC_SERIE)    
    if (CC_SERIE == "''" ):    
        for p in lst_path_regerado:
...
if __name__ == "__main__":
    ...
    if (ret == 0 ): 
        ...
        ret = todaspastas(CC_IE,CC_FILIAL,DT_MESANO_INICIO,DT_MESANO_FIM,CC_SERIE,UFI)
