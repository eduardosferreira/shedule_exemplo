...
def processar():
    ...
    if ret == 0 : 
        if not atualizaStatusRobo(id_serie, 22) :
            ret = 8
...
def atualizaStatusRobo(id_serie, status):
    ...
    try:
        ...
        try:
            ...
            ### Clone2
            id_clone = 42
            if configuracoes.maquina == 'clone1' :
                id_clone = 41
            if configuracoes.maquina == 'clone6' :
                id_clone = 21
            if configuracoes.maquina == 'clone7' :
                id_clone = 44
            
           
            cursor.execute(" UPDATE FILA         F                                            " + chr(10) +
                           "    SET F.fk_id_clone = %s                                        "%(id_clone) + chr(10) +
