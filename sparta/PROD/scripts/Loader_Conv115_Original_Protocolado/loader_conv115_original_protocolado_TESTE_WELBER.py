...
----------------------------------------------------------------------------------------------
  ...
  DESCRICAO : Processo para realizar a carga dos arquivos de Mestre, Itens e Cadastro .
                Parametros :
                    - UF 
                    - Periodo
                    - Serie ( Opcional )
...
listaCadastroProtocolado = []

def processar():     
            
    global listaMestreProtocolado
    global listaCadastroProtocolado 

    comum.carregaConfiguracoes(configuracoes)

    log("Iniciando carga do(s) cadastro(s) a partir do(s) arquivo(s) Protocolado(s) ...")    
    ...
    comando = """
        SELECT TO_CHAR(l.MES_ANO, 'MM')    MES
            ...
            , '/portaloptrib/CONV39/' ||TO_CHAR(l.MES_ANO, 'YYYY_MM')||'/s'||REPLACE(SERIE,' ','')  DIRETORIO
...
"""%( serie )
    ...
    try :
        ...
        # records = [
        #     [ '07', '2018', 'U  T', 'UT', '0001', 'SP', '0000000001', '/portaloptrib/CONV39/2018_07/sUT' ],
        #     [ '07', '2018', 'U K', 'UK', '0001', 'SP', '0000000001', '/portaloptrib/CONV39/2018_07/sUK' ],
        #     [ '07', '2018', '1', '1', '0001', 'SP', '0000000001', '/portaloptrib/CONV39/2018_07/s1' ],
        #     [ '07', '2018', 'C', 'C', '0001', 'SP', '0000000001', '/portaloptrib/CONV39/2018_07/sC' ]
        # ]
    ...
    while records :
        ...
        # dir_protocolado  = dir_serie + '/PROTOCOLADO'
        dir_protocolado  = dir_serie + '/OBRIGACAO'
        ...
        for arq in lst_dir :
            if len(arq) > pos and os.path.isfile( os.path.join( dir_protocolado, arq ) ) : 
                ...
                if arq[pos] == 'M' :
                    ...
                    list_arqs_cadastro.append(arq)
        
        if len(list_arqs_mestre) != len(list_arqs_itens) or len(list_arqs_mestre) != len(list_arqs_cadastro) :
        ...
        th = threading.Thread( target = cargaProtocolado, args = ( dir_protocolado, list_arqs_cadastro, ano_aaaa, mes, serie, uf, id_serie, os.getpid() ) )
        th.start()
        lst_threads.append(th)
        time.sleep(2)
...
"""%(arq)
        os.system("cat %s.log | grep -e 'ORA-' -e 'Rejected' -e 'SQL.Loader-' > Resumo_%s.log"%(arq_ctl[:-4], arq_ctl[:-4])) 
