...
def atualizaStatusRobo(id_serie, status):
    ...
    try:
        ...
        try:
            ...
            log(" ")    
            log("-------------------------------------------------------------------------")    
            log("- Atualizando Servidor para Clone 7...")    
            log(" ")    
            ...
            cursor.execute(" UPDATE FILA         F                                            " + chr(10) +
                           "    SET F.fk_id_clone = 42                                        " + chr(10) +
                           "  WHERE F.ID_SERIE_LEVANTAMENTO = :ID_SERIE                       " 
                          ,(id_serie,))
...
def movimentaObrigacao():
    ...
    if not os.path.isdir(diretorioDestino) :
        log("Criando Diretorio de destino...")
    ...
    if  ano <= 16:
        ...
        for arq in listaArquivos:
            ...
            if  os.path.isfile(arq):
                ...
                log("### Arquivo NAO ENCONTRADO ... %s"%( arq ))
...
def quebraVolume():
    if configuracoes.dic_dados_serie['quebra_volumes']:
        ...
        os.chdir( os.path.join(dir_base, 'scripts', 'redistribuir_volumes_conv115') )
