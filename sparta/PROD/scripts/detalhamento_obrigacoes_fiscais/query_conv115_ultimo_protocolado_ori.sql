SELECT 
  ...
  FROM gfcarga.TSH_ITEM_CONV_115[C1]      I
  JOIN gfcarga.TSH_SERIE_LEVANTAMENTO[C1] L
    ON L.id_serie_levantamento  = I.id_serie_levantamento
   AND L.uf_filial             	= I.uf_filial
  JOIN gfcarga.TSH_CONTROLE_CONV_115[C1] C
   ...
   and i.volume = c.VOLUME 
 WHERE L.uf_filial           	= '[uf_filial]'
   AND l.mes_ano               	= to_date('[datai]', 'DD/MM/YYYY')
   AND l.ORIGEM_PROTOCOLADO    	= 'ORIGINAL'
   AND I.sit_doc   		= 'N'
 GROUP BY L.EMPS_COD, L.FILI_COD, I.SERIE, I.ALIQUOTA,I.CFOP,I.VOLUME,C.NOMENCLATURA_ARQ_NF,C.HASHCOD_NF,c.STATUS_RETIFICACAO_NF
